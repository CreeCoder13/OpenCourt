...
export function CaseListCard({ item, matchReason }: { item: CaseRecord; matchReason?: string }) {
  ...
  return (
    <article className="result-card">
      ...
      <div className="result-bottom">{isOngoing ? <CaseStatusBadge status={item.currentStatus || item.status} /> : <OutcomeBadge outcome={item.outcome} />}<Link className="card-link-inline" href={`/cases/${item.slug}`}>View Case →</Link></div>
    </article>
  );
}

...
export default function TopicsPage() { return <PageShell><section className="page-hero"><Breadcrumbs items={[{ label: "Home", href: "/" }, { label: "Legal Definitions" }]} /><div className="page-hero-grid"><div><p className="kicker">Plain-language legal dictionary</p><h1>Legal definitions</h1></div><p>Choose a legal term for a dedicated plain-language page showing its meaning, connected judgments, chronological development, and related concepts. “Indigenous law” and “Aboriginal law” are not treated as synonyms.</p></div></section><section className="catalog-section"><div className="catalog-intro"><h2>Browse {topics.length} legal definitions</h2><p>Each definition is checked against a verified primary legal authority.</p></div><div className="topic-catalog">{topics.map((topic, index) => <Link href={`/topics/${topic.slug}`} key={topic.id}><span>{String(index + 1).padStart(2, "0")}</span><h2>{topic.name}</h2><p>{topic.description}</p><div><small>Verified · {topic.definitionSources.length} primary source{topic.definitionSources.length === 1 ? "" : "s"}</small><b>→</b></div></Link>)}</div></section></PageShell>; }

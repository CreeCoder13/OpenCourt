...
export default async function TreatyPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const sections = ["overview", "context", "parties", "territory", "terms", ...(item.adhesions.length ? ["adhesions"] : []), "cases", "sources"];
  return <PageShell>
    ...
    <div className="treaty-record-layout">
      ...
      <article className="treaty-record-main">
        ...
        <section id="parties"><p className="section-number">03 · The parties</p><h2>Who entered into the agreement</h2><div className="treaty-party-columns"><div><h3>Indigenous parties</h3>{item.indigenousParties.map((party) => party.communitySlug ? <Link href={`/communities/${party.communitySlug}`} key={party.name}><span>{party.name}</span><small>{party.role || "Party"} · Community profile →</small></Link> : <div className="treaty-party" key={party.name}><span>{party.name}</span><small>{party.role || "Party"}</small></div>)}</div><div><h3>Crown / government parties</h3>{item.crownParties.map((party) => <div className="treaty-party" key={party}><span>{party}</span><small>Historically described party</small></div>)}</div></div><p className="verification-copy">A Nation or community is linked only when the relationship has been identified in the reviewed record. This list does not infer treaty relationships from geography alone.</p></section>
        ...
        <section id="terms"><p className="section-number">05 · Treaty terms</p><h2>Key topics in plain language</h2><p>These topic summaries orient the reader to the record. They do not replace the treaty text, oral promises, amendments, implementation agreements or legal decisions.</p><div className="treaty-terms-grid">{item.terms.map((term) => <div key={term.topic}><span>{String(item.terms.indexOf(term) + 1).padStart(2, "0")}</span><h3>{term.topic}</h3><p>{term.summary}</p></div>)}</div></section>

...
export default function Home() {
  return <><SiteHeader /><main>
    ...
    <section className="topics-home"><div className="section topics-home-inner"><div className="section-heading"><div><p className="kicker">Plain-language legal dictionary</p><h2>Legal definitions</h2></div><p>Choose a legal concept, then follow the cases that established, expanded, clarified, or limited it.</p><Link className="text-link" href="/topics">Explore all definitions →</Link></div><div className="home-topic-grid">{topics.map((topic, index) => <Link href={`/topics/${topic.slug}`} key={topic.id}><span>{String(index + 1).padStart(2, "0")}</span><b>{topic.name}</b><i>→</i></Link>)}</div></div></section>

...
export default function Home() {
  return <><SiteHeader /><main>
    ...
    <section className="section landmarks-section"><div className="section-heading"><div><p className="kicker">Start here</p><h2>Landmark cases</h2></div><p>Explore decisions that shaped Canadian law concerning Indigenous peoples and Aboriginal and treaty rights.</p><Link className="text-link" href={homeSectionLinks.allCases}>View all cases →</Link></div><div className="case-grid">{landmarks.map((item) => item && <Link href={`/cases/${item.slug}`} className="case-card" key={item.id}><div className="case-meta"><span>{new Date(`${item.decisionDate}T00:00:00`).getFullYear()}</span><span className="topic-pill">{item.legalTopics[0]}</span></div><h3>{item.caseName}</h3><p className="citation">{item.officialCitation}</p><p>{item.summaryShort}</p><span className="card-link">Read the plain-language summary <b>→</b></span></Link>)}</div></section>

    <section className="topics-home"><div className="section topics-home-inner"><div className="section-heading"><div><p className="kicker">Plain-language legal dictionary</p><h2>Legal definitions</h2></div><p>Choose a legal concept, then follow the cases that established, expanded, clarified, or limited it.</p><Link className="text-link" href={homeSectionLinks.allDefinitions}>Explore all definitions →</Link></div><div className="home-topic-grid">{topics.map((topic, index) => <Link href={`/topics/${topic.slug}`} key={topic.id}><span>{String(index + 1).padStart(2, "0")}</span><b>{topic.name}</b><i>→</i></Link>)}</div></div></section>

...
export async function POST(request: Request) {
  ...
  let options: { queryOffset?: number; queryLimit?: number; processLimit?: number; topic?: string; year?: number; ongoing?: boolean; dryRun?: boolean; mode?: "incremental" | "broad" | "backfill" } = {};
  ...
  if (options.mode !== undefined && !["incremental", "broad", "backfill"].includes(options.mode)) return Response.json({ error: "mode must be incremental, broad, or backfill" }, { status: 400 });

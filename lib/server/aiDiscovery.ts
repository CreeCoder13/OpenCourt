...
import { fetchWithRetry } from "../discovery/http";
...
const classificationSchema = {
  ...
  required: ["relevance", "confidence", "recordType", "categories", "proposedTitle", "summary", "significanceSignals", "nations", "citations", "verificationNeeded", "court", "courtFileNumber", "decisionDate", "neutralCitation", "legislationCitation", "parties", "constitutionalSections", "legislationReferenced", "casesCited", "treatiesReferenced", "impactSignals", "proceduralStage", "latestDevelopment", "latestDevelopmentDate", "upcomingHearingDate"],
  properties: {
    ...
    court: { type: ["string", "null"] }, courtFileNumber: { type: ["string", "null"] }, decisionDate: { type: ["string", "null"] }, neutralCitation: { type: ["string", "null"] }, legislationCitation: { type: ["string", "null"] },
    ...
    proceduralStage: { type: ["string", "null"] }, latestDevelopment: { type: ["string", "null"] }, latestDevelopmentDate: { type: ["string", "null"] }, upcomingHearingDate: { type: ["string", "null"] },
  },
} as const;
...
export async function classifyLegalDocument(input: { contentHash: string; url: string; title?: string; text: string }): Promise<AiClassification> {
  ...
  const cached = await getCachedAi(input.contentHash, "legal-classification-v2", model);
  ...
  await cacheAi(input.contentHash, "legal-classification-v2", model, parsed as unknown as Record<string, unknown>, Number(usageBody?.input_tokens ?? 0), Number(usageBody?.output_tokens ?? 0));
  return parsed;
}
